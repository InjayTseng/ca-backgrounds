...
export default {
  ...
  concept: {
    ...
    dies: '不會。決定性、無限生成；30 和 90 從單點種子長出三角碎形，110 和 184 從隨機列開始。',
  ...
  create(canvas, env) {
    ...
    function resize() {
      ...
      cols = Math.ceil(canvas.width / cs); rows = Math.ceil(canvas.height / cs);
    ...
    return {
      ...
      pointer(p) { if (p.down) { const x = Math.floor(p.x * (canvas.width / canvas.clientWidth) / cs); for (let i = -2; i <= 2; i++) row[(x + i + cols) % cols] ^= 1; } },
